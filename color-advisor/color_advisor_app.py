...
def verify_login(claude_bin):
    ...
    Returns (True, message) when logged in, (False, reason) otherwise.
    """
    try:
...
def open_login_window(claude_bin):
    """Open an interactive `claude` session in a new terminal so the user
    can complete browser login."""
    try:
        if sys.platform.startswith("win"):
            subprocess.Popen(
                [claude_bin], creationflags=subprocess.CREATE_NEW_CONSOLE
            )
...
def run_gui():
    ...
    state = {"claude_bin": None, "logged_in": False}
    ...
    analyze_btn = tk.Button(frm, text="Get color recommendation", width=26, state="disabled")
    analyze_btn.grid(row=3, column=0, pady=6)
    ...
    output.grid(row=5, column=0, sticky="nsew", pady=4)
