...
export async function page(ctx) {
  ...
  const counted = commitments.filter((c) => c.status !== 'Cancelled').length;
  const unexplained = commitments.filter(
    (c) => !['Completed', 'Cancelled'].includes(c.status) && !c.carryover_reason);
    ...
    <div class="section">
      <div class="card">
        <div class="card-body">
          ...
          <div class="stat-row" style="margin:15px -16px -16px;border-radius:0 0 12px 12px;overflow:hidden;border-bottom:0;border-top:1px solid var(--border)">
            <div class="stat"><div class="v">${done}/${counted}</div><div class="k">Commitments</div></div>
    ...
    <div class="section grid grid-2" style="align-items:start">
      <div class="card">
        <div class="card-head">
          ...
          <span class="sub">${counted} of ${max} recommended</span>
          <span class="spacer"></span>
          <button class="btn btn-primary btn-sm" data-action="add-commitment"${counted >= max ? ' title="Already at the recommended daily limit"' : ''}>
        ...
        : `<div class="check-list">${commitments.map(checkItem).join('')}</div>`}
        ${unexplained.length ? `<div class="card-body" style="border-top:1px solid var(--border)">
          <div class="callout warn">${U.icon('alert', 16)}<div>
            <b>${unexplained.length} unfinished item${unexplained.length > 1 ? 's need' : ' needs'} an explanation.</b>
      ...
      <div class="stack">
        <div class="card">
          <div class="card-head">
            <h2>Log progress</h2>
            <span class="sub">Measurable results only</span>
          </div>
          <div class="card-body tight">
            ${data.my_projects.length ? data.my_projects.map((p) => `
  ...
  return {
    ...
    mount(root) {
      ...
      root.addEventListener('click', async (e) => {
        ...
        if (editId) {
          const c = commitments.find((x) => x.id === Number(editId));
          commitmentForm(c);
        }
        if (statusId) setStatus(commitments.find((x) => x.id === Number(statusId)));
        if (toggleId) {
...
function setStatus(c, forceReason = false) {
