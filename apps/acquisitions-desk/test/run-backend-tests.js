...
test('roles: REP cannot manage users/settings/reassign, TECHNICAL cannot work leads, MANAGER can assign', () => {
  ...
  ok(ctx.assignLead(leadB.lead_id, me.user_id, null), 'rep can claim'); bad(ctx.restoreLead(leadB.lead_id, 'NEW', null), 'ACCESS_DENIED', 'rep cannot restore'); bad(ctx.getJuanDashboard(), 'ACCESS_DENIED', 'rep no team view');
