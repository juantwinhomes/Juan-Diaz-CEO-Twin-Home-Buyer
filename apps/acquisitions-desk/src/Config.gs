...
var HEADERS = {
  ...
  LEADS: ['lead_id','address','seller_name','phone','source','equity_note','status','assigned_to','team',
          ...
          'created_at','updated_by','updated_at','last_touched_at','version','exit_strategy','disposition','purchase_price','closing_date','sale_price','seller_email','assignment_fee','revenue'],
...
var LEAD_STATUS = {
  ...
  ARCHIVED_CONTRACT_CANCELLED: 'ARCHIVED_CONTRACT_CANCELLED', ARCHIVED_SOLD: 'ARCHIVED_SOLD',
...
var STATUS_LABELS = {
  ...
  ARCHIVED_CONTRACT_CANCELLED: 'Archived: contract cancelled', ARCHIVED_SOLD: 'Archived: sold',
...
var STATUS_ORDER = ['NEW','INVESTIGATING','CONTACT_MADE','APPOINTMENT_SET','UNDER_CONTRACT','CLOSED',
                    'ARCHIVED_CONTRACT_CANCELLED','ARCHIVED_SOLD','ARCHIVED_NO_EQUITY','ARCHIVED_NOT_INTERESTED','ARCHIVED_BAD_DATA'];
var LIVE_STATUSES = ['NEW','INVESTIGATING','CONTACT_MADE','APPOINTMENT_SET','UNDER_CONTRACT'];
/** Dead outcomes. A cancelled contract is first because it is the one that cost us something. */
var ARCHIVED_STATUSES = ['ARCHIVED_CONTRACT_CANCELLED','ARCHIVED_SOLD','ARCHIVED_NO_EQUITY','ARCHIVED_NOT_INTERESTED','ARCHIVED_BAD_DATA'];
...
var EXIT_STRATEGIES = ['Wholesale - Assignment','Wholesale - Double Close','Wholetail','Fix & Flip','Wholetail / Flip','Hold'];
/** On an assignment we never own the house: the fee is the whole of the money, and there is no resale price. */
var EXIT_ASSIGNMENT = 'Wholesale - Assignment';
/** Values written before Wholesale was split in two. Still accepted so an old record can be saved and corrected. */
var EXIT_STRATEGIES_LEGACY = ['Wholesale'];
...
var LEAD_NUMERIC = ['arv','repairs','asking_price','offer','purchase_price','sale_price','assignment_fee','revenue','contact_attempts','version'];
