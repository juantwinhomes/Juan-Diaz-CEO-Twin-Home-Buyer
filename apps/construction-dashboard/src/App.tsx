...
export default function App() {
  ...
  const taskList = (rows: { p: Property; t: Task }[], empty: string) =>
    rows.length ? (
      <><TaskListHead />{rows.map(({ p, t }) => (
        <TaskCard key={`${p.id}-${t.id}`} task={t} propertyLabel={short(p)} onSave={saveTask(p)} onOpenProperty={() => openProperty(p.id)}
  ...
  const tiles: [string, number, string, View | null, string?][] = [
    ["Active properties", s.properties, "text-ink", "all"],
    ["Tasks requiring action", s.action, "text-urgent", "action"],
    ["Blocked tasks", s.blocked, "text-urgent", "waiting"],
    ["Waiting on vendor / third party", s.waiting, "text-wait", "waiting"],
    ["Inspections pending", s.inspections, "text-info", "inspections"],
    ["Approvals needed", s.approvals, "text-approve", "juan"],
    ["Completed tasks", s.completed, "text-ok", "completed", s.archived ? `+${s.archived} archived on Monday` : undefined],
  ];

  let body: React.ReactNode;
  if (!ready) {
    body = <div className="rounded-lg border border-line bg-surface p-8 text-center text-ink2">{note || "Loading the property board…"}</div>;
  } else if (current) {
    ...
    body = <div className="rounded-lg border border-line bg-surface p-6">No property with id “{openId}”. <button className="font-semibold text-accent" onClick={back}>Back to all properties</button></div>;
  } else if (view === "all") {
    ...
    body = (
      <div className="space-y-4">
        ...
        <div className="flex items-baseline justify-between">
          <h2 className="font-display text-[18px] font-bold tracking-tight">{active ? "Matching properties" : "Properties"}</h2>
    ...
    body = (
      <Section title="Action Needed Now — all properties" count={rows.length} tone="urgent" hint="Urgent, blocked, awaiting approval, or past due">
        {rows.length ? <div className="grid gap-2.5 p-3 lg:grid-cols-2">{rows.map(({ p, t }) => <ActionCard key={`${p.id}-${t.id}`} t={t} property={p.address} onOpen={() => openProperty(p.id)} />)}</div>
          : <Empty>Nothing needs immediate action.</Empty>}
      </Section>
    );
  } else if (view === "juan") {
    const juanTasks = sortRows(all.filter(({ t }) => isJuanApproval(t)));
    const decisions = properties.flatMap((p) => openDecisions(p).map((d) => ({ ...d, property: p.address })));
    ...
    const items = properties.flatMap((p) => p.inspections.map((i) => ({ ...i, property: short(p) })));
    ...
    const items = properties.flatMap((p) => p.purchases.map((x) => ({ ...x, property: short(p) })));
    ...
    const events = properties.flatMap((p) => p.upcomingEvents.map((e) => ({ ...e, property: short(p) })));
    const overdue = all.filter(({ t }) => isOverdue(t, today)).map(({ p, t }) => ({ date: t.deadline!, title: t.title, owner: t.owner, property: short(p) }));
        ...
        <Section title="Archived on Monday (no item detail supplied)" count={s.archived} defaultOpen>
          <ul className="divide-y divide-line">
            {properties.filter((p) => p.archivedCompleted).map((p) => (
              <li key={p.id} className="flex justify-between px-4 py-2 text-[13px]"><span>{p.address}</span><span className="num font-mono">{p.archivedCompleted}</span></li>
            ))}
          </ul>
        </Section>
      </div>
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 pb-16 sm:px-6">
      <header className="flex flex-wrap items-end justify-between gap-3 pt-5 pb-3">
        <div>
          <div className="eyebrow">Equity Track Inc · Twin Home Buyer</div>
          <h1 className="font-display text-[26px] font-extrabold leading-none tracking-tight sm:text-[30px]">Construction Control</h1>
        </div>
        <div className="flex flex-wrap items-center gap-3 text-[12px] text-ink2">
          <span className="inline-flex items-center gap-1.5">
            <i className={`block h-2 w-2 rounded-full ${mode === "live" ? "bg-ok" : mode === "loading" ? "bg-line2" : "bg-high"}`} />
          ...
          {mode === "local" && localEdits > 0 && (
            <button type="button" onClick={resetLocal} className="rounded border border-line2 px-2 py-0.5 text-[11.5px] hover:text-urgent">Discard {localEdits} local edit{localEdits === 1 ? "" : "s"}</button>
          )}
      ...
      {staleDays > 7 && (
        <div className="mb-3 rounded-md border border-high/30 bg-high-soft px-4 py-2.5 text-[13px] text-high">
          <b>Board data is {staleDays} days old.</b> Deadlines before today show as past due until newer Monday.com updates are loaded.
        </div>
      )}

      <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
        {tiles.map(([label, n, color, v, sub]) => (
          <button key={label} type="button" onClick={() => v && pickView(v)}
            className={`rounded-lg border bg-surface px-3 py-2.5 text-left transition-colors hover:border-line2 ${!current && view === v && v !== "all" ? "border-accent" : "border-line"}`}>
            <div className={`num font-mono text-[26px] font-semibold leading-none ${color}`}>{n}</div>
            <div className="mt-1.5 text-[11.5px] font-medium leading-tight text-ink2">{label}</div>
            {sub && <div className="text-[10.5px] text-ink3">{sub}</div>}
          </button>
        ))}
      </div>

      <nav className="sticky top-[env(safe-area-inset-top,0px)] z-10 -mx-4 mb-4 overflow-x-auto border-y border-line bg-ground/95 px-4 backdrop-blur sm:-mx-6 sm:px-6" aria-label="Quick views">
        <div className="flex gap-1 py-1.5">
          {VIEWS.map(([id, label]) => {
            const on = !current && view === id;
            return (
              <button key={id} type="button" onClick={() => pickView(id)} aria-current={on ? "page" : undefined}
                className={`whitespace-nowrap rounded px-3 py-1.5 text-[13px] font-semibold ${on ? "bg-accent text-white" : "text-ink2 hover:bg-surface hover:text-ink"}`}>
                {label}
              </button>
            );
          })}
      ...
      <footer className="mt-10 border-t border-line pt-3 text-[11.5px] text-ink3">
